...
- **Not in this plan (spec requirements silently dropped, flagged by final review — carry into Phase B or a follow-up plan):** the spec's webapp section calls for URL-querystring-based filter state, dark mode, and mobile responsiveness (see spec's "프론트엔드" section); Task 9 shipped none of these and this plan never recorded them as deferred. There is also no CSS at all in Task 9's output. None of this blocks Phase A's own scope (fixture-driven pipeline + a functional, if unstyled, webapp), but it should not be forgotten before this project is shown to anyone.
